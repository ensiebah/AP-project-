...
public class ConversationServiceImpl implements ConversationService {
    ...
    public ConversationDto createConversation(Long buyerId, Long advertisementId) {
        ...
        if (existingConversation.isPresent()) {
            // 🟢 پاس دادن buyerId به عنوان ورودی دوم متد مپر
    ...
    public List<ConversationDto> getUserConversations(Long userId) {
        ...
        return conversations.stream().map(c -> mapToDto(c, userId)).toList();
    }


    private ConversationDto mapToDto(Conversation conversation, Long currentUserId) {
        // پیدا کردن نام طرف مقابل بر اساس اینکه کاربر لاگین شده خریدار است یا فروشنده
        String opponentUsername = conversation.getBuyer().getId().equals(currentUserId)
                ? conversation.getSeller().getUserName()
                : conversation.getBuyer().getUserName();

        // واکشی پیام‌های این چت برای پیدا کردن آخرین پیام
        ...
        if (messages != null && !messages.isEmpty()) {
            // چون پیام‌ها با OrderBySentAtAsc واکشی شده‌اند، آخرین عنصر لیست همان آخرین پیام است
            Message lastMessage = messages.get(messages.size() - 1);
            lastMessageContent = lastMessage.getText();
        ...
        return ConversationDto.builder()
                ...
                // 🟢 فیلدهای جدیدی که برای باکس چت فرانت‌اَند نیاز داری:
